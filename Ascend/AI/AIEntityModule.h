...
protected:
	...
	UPROPERTY(VisibleAnywhere)
	UAttributeComponent* AIAttributeComponent;

	UPROPERTY(VisibleAnywhere)
	ULevelSystemComponent* LevelSystem;

public:
	USkeletalMeshComponent* GetAISkeletalMesh() const
	{
		return AIMesh;
	}

	UCapsuleComponent* GetAICapsuleComponent() const 
	{ 
		return AICapsuleComponent;
	}

	UAttributeComponent* GetAIAttributeComponent() const
	{
		return AIAttributeComponent; 
	}

	ULevelSystemComponent* GetAILevelSystemComponent() const
	{
		return LevelSystem;
	}
...
private:
	UPROPERTY()
	ABaseCharacter* LevelCharacter;
public:
	virtual float GetExperience();

	void SetLevelCharacter(ABaseCharacter* InLevelCharacter)
	{
		LevelCharacter = InLevelCharacter;
	}

	ABaseCharacter* GetLevelCharacter() const
	{
...
private:
	...
	void SpawnExperiencePoints(const FVector& SpawnLocation);

protected:
	UPROPERTY(EditAnywhere, Category = "Developer Settings")
	TSubclassOf<AExperiencePointPickup> ExperiencePointPickupClass;
#pragma endregion
