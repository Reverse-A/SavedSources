...
public:
	void UpdateWeaponProperties(ERangedWeaponType WeaponType)
	{
		switch (WeaponType)
		{
...
USTRUCT(BlueprintType)
struct FRangeWeaponImages
{
	GENERATED_BODY()
protected:

	UPROPERTY(EditAnywhere)
	UTexture2D* PistolImage;

	UPROPERTY(EditAnywhere)
	UTexture2D* RifleImage;

	UPROPERTY(EditAnywhere)
	UTexture2D* SubmachineGunImage;

	UPROPERTY(EditAnywhere)
	UTexture2D* HeavyMachineGunImage;

	UPROPERTY(EditAnywhere)
	UTexture2D* ShotgunImage;

	UPROPERTY(EditAnywhere)
	UTexture2D* SniperRifleImage;

	UPROPERTY(EditAnywhere)
	UTexture2D* BowImage;

	UPROPERTY(EditAnywhere)
	UTexture2D* CrossbowImage;

	UPROPERTY(EditAnywhere)
	UTexture2D* ThrowingWeaponImage;

	UPROPERTY(EditAnywhere)
	UTexture2D* LauncherImage;

	UPROPERTY(EditAnywhere)
	UTexture2D* CannonImage;

public:
	UTexture2D* GetPistolImage() const
	{
		return PistolImage;
	}

	UTexture2D* GetRifleImage() const
	{
		return RifleImage;
	}

	UTexture2D* GetSubmachineGunImage() const
	{
		return SubmachineGunImage;
	}

	UTexture2D* GetHeavyMachineGunImage() const
	{
		return HeavyMachineGunImage;
	}

	UTexture2D* GetShotgunImage() const
	{
		return ShotgunImage;
	}

	UTexture2D* GetSniperRifleImage() const
	{
		return SniperRifleImage;
	}

	UTexture2D* GetBowImage() const
	{
		return BowImage;
	}

	UTexture2D* GetCrossbowImage() const
	{
		return CrossbowImage;
	}

	UTexture2D* GetThrowingWeaponImage() const
	{
		return ThrowingWeaponImage;
	}

	UTexture2D* GetLauncherImage() const
	{
		return LauncherImage;
	}

	UTexture2D* GetCannonImage() const
	{
		return CannonImage;
	}
};
...
protected:
	...
	void InitialRangeWeaponProperties();
...
protected:
	...
	UPROPERTY(EditAnywhere)
	FRangeWeaponImages RangedWeaponImages;
public:
	FRangedWeaponProperties GetRangedWeaponProperties();

	ERangedWeaponType GetRangedWeaponType();

	void SetWeaponFireMode(EWeaponFiringMode newFireMode);

	void SetWeaponFireType(EFireType newFireType);
